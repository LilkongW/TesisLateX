...
\begin{figure}[htbp]
	...
	\caption[Esquema de estimación del vector de mirada en 3D.]{Esquema de la estimación del vector de mirada. (Izquierda) Identificación del contorno pupilar.
...
El modelo es capaz de re-adquirir la posición del ojo instantáneamente \textit{frame} a \textit{frame} específicamente para el \textit{dataset} de esta investigación, garantizando la continuidad de las series temporales necesarias para el cálculo de la velocidad y aceleración.
...
\begin{figure}[H]
	...
	\caption[Correspondencia entre imagen infrarroja y modelo 3D ocular.]{Correspondencia entre la señal visual capturada y su representación 
		computacional. \textbf{Izquierda:} imagen infrarroja del ojo adquirida por la 
		cámara, donde se aprecia la pupila detectada. 
		\textbf{Derecha:} modelo tridimensional resultante, en el que el diámetro 
		pupilar estimado determina la escala del globo ocular y el vector de mirada 
		(\textit{gaze vector}) que define su orientación en el espacio 3D. Esta 
		transformación constituye el núcleo del \textit{pipeline} de estimación oculomotora 
		propuesto.}
