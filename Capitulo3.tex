...
Esta elección metodológica se justifica por la naturaleza física y medible de las variables bajo estudio: posiciones espaciales del ojo expresadas en coordenadas $(x, y)$, velocidades angulares (en $^\circ/\text{s}$), aceleraciones, y otras métricas cinemáticas que describen la dinámica del sistema oculomotor.
...
\begin{itemize}
	...
	\item Velocidad angular del movimiento ocular $v(t)$ [$^\circ/\text{s}$]
	\item Aceleración angular $a(t)$ [$^\circ/\text{s}^2$]
	\item Índice de suavidad del movimiento (jerk) $J(t)$ [$^\circ/\text{s}^3$]
...
\begin{align}
	V(t) &= \frac{d}{dt} \text{SavGol}(P(t)) \quad [^\circ/\text{s}] \\
	a(t) &= \frac{d}{dt} V(t) \quad [^\circ/\text{s}^2]
\end{align}
