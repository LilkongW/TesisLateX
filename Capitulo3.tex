...
\begin{itemize}
	...
	\item \textbf{Precio}: 15 USD (Amazon)
\end{itemize}
...
El filtro Gaussiano es un filtro lineal de paso bajo que atenúa las componentes de alta frecuencia espacial (ruido) mediante una convolución con un \textit{kernel} gaussiano bidimensional descrito por Giménez et al. \cite{gimenez2016aplicacion}:
...
En este estudio se utilizó $\sigma = 1.5$ y un \textit{kernel} de tamaño $5 \times 5$ píxeles.
...
\begin{enumerate}
	\item \textbf{Suavizado Gaussiano}: Se aplicó un filtro Gaussiano con \textit{kernel} $(7 \times 7)$ para reducir el ruido de alta frecuencia del sensor CMOS.
...
Las métricas de esta categoría describen la dinámica del movimiento 
sacádico a distintas escalas de derivación temporal:
...
Su justificación biométrica se fundamenta en el 
trabajo de Komogortsev et al.\ \cite{komogortsev2010}, quienes 
demostraron que los parámetros del modelo de la planta oculomotora 
---incluyendo la velocidad y la aceleración--- son lo suficientemente 
estables inter-sesión e idiosincrásicos inter-sujeto como para 
constituir una firma biométrica. En particular, el \textit{Jerk} 
(derivada de la aceleración, $J = \dddot{\theta}$) es especialmente 
relevante porque cuantifica el costo energético del movimiento:

\begin{equation}
	J(t) = \frac{d^3\theta}{dt^3} \quad [\text{°/s}^3]
	\label{eq:jerk}
\end{equation}

El cerebro minimiza el \textit{jerk} cuadrático integrado al planificar movimientos oculares óptimos \cite{leigh2015neurology}, y la forma exacta en que cada individuo alcanza ese mínimo está determinada por la geometría muscular única de su sistema oculomotor \cite{bahill1975}. Así, \texttt{Jerk\_Max} captura el límite superior del esfuerzo neuromuscular, mientras que \texttt{Jerk\_Mean} describe la estrategia promedio de control motor. La métrica \texttt{Velocity\_Transition\_Smoothness} complementa estas variables midiendo la continuidad de la curva de velocidad, un indicador de la calidad del control motor de lazo cerrado que varía entre individuos con distinta coordinación oculomotora \cite{vanopstal1987skewness}.
...
Las métricas de esta categoría describen la organización temporal de 
la exploración visual: cuántos movimientos se producen, con qué 
frecuencia, y qué tan eficientemente se realizan:
...
La tasa de microsacadas (\texttt{Microsaccade\_Rate}) es particularmente relevante como descriptor de biometría conductual: los microsacadas son movimientos involuntarios de fijación donde su frecuencia y dirección están moduladas por factores atencionales y neurofisiológicos propios de cada individuo \cite{holmqvist2011eye}. La pendiente de la Secuencia Principal (\texttt{Main\_Seq\_Slope}) cuantifica la eficiencia neuromuscular del sistema sacádico como fue descrito en la Sección~\ref{subsec:justificacion_metricas}, y su inclusión como métrica independiente permite capturar variaciones individuales en la curva velocidad-amplitud que el modelo exponencial completo de Bahill \cite{bahill1975} no resume en un único escalar. La velocidad durante fijación (\texttt{Fixation\_Vel\_Mean}) captura la micro-deriva residual del ojo durante los periodos de aparente reposo, un marcador de la estabilidad del sistema de mantenimiento de fijación que varía entre individuos sanos \cite{leigh2015neurology}.
